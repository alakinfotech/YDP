...
@synthesize targetForExecution;
@synthesize objectForExecution;
...
// Stop the activity indicator when showing the HUD
- (void)showHUDWithoutIndicator:(BOOL)animated {
	useAnimation = animated;
...
- (void)launchExecution {
	...
	[self performSelectorOnMainThread:@selector(cleanUpByUser) withObject:nil waitUntilDone:NO];
    //[pool release];
}
